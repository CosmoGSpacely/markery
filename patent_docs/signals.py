"""
signals.py — enrich candidates.jsonl with text-matching signals.
...
def _name_hit(text: str | None, mark: str) -> bool:
    if not text:
        return False
    # Significant words in the mark (len > 3, non-stopword)
...
def _fetch_patent_texts(conn_pat: duckdb.DuckDBPyConnection,
                        patent_no: str) -> tuple[str | None, str | None]:
