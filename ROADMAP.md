...
1. Build an annual review per year at `site/<annual-review-project>/<year>/`: a year landing page linking the twelve monthly design-mark galleries (mark_draw_cd LIKE '3%', by filing month), rendered in the site chrome (global bar) so it sits under the Markery portal. The years come from the annual-review **project**'s `review_years` config — the build is driven by the project, not a hardcoded list.
...
Correction 2026-06-23: The first pass generated reviews from a hardcoded `REVIEW_YEARS` in `build.py` and left the source project untouched — the review was supposed to *be* a project. Fixed: added `ProjectType.ANNUAL_REVIEW` + a `review_years` config field; **renamed `monthly-image-review` → `annual-design-review`** and retyped it (`{"type":"annual-review","review_years":[1929,1930]}`), removed its stale monthly `output/`, and rewrote its README/STATUS. `build_all` now discovers annual-review projects and builds each one's `review_years` into `site/<project>/<year>/` (was `site/reviews/<year>/`); portal cards link there. Verified: `site build-all` → `site/annual-design-review/{1929,1930}/`, `site check` clean (4823 pages, 0 broken/orphans); full suite 800 green.
...
P4 PASSED when: annual design-mark reviews exist for 1929 and 1930 (year landing + monthly galleries) under `site/<annual-review-project>/<year>/`, driven by the annual-review project's `review_years`, each surfaced as a card on the Markery root portal, with `markery site check` green across the root. — PASSED (2026-06-22; reworked 2026-06-23 to be project-driven via the `annual-design-review` project)
