...
Running `markery trademark enrich <project>` fetches case status records from the USPTO TSDR API for confirmed pairs and candidates in the uncertainty band. For each successfully fetched record, `mark_case_status.goods_desc` is populated in `trademarks.duckdb`.
...
`trademarks.duckdb.mark_case_status` — one row per serial number. Queryable via:

```sql
SELECT goods_desc FROM mark_case_status WHERE serial_no = '71246709'
```

## Request to researcher

**Human-readable:**
> "I need the goods/services description for trademark [serial_no]. Please run: `markery trademark enrich information-systems`"
