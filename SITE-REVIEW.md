...
Remaining in P1: the broader **responsive / semantic-landmark / SEO-metadata** pass (P1 step 5) and its tests. Trademark images / patent figures are now fetched for precision-tools; further coverage is bounded by what TSDR/EPO actually hold for these pre-1940 records (a data/enrichment matter — initial acquisition is existing `trademark enrich` / `patent fetch` tooling; ongoing review is Phase 24 P4, enhancement quality is P5).
